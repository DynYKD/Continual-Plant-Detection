...
def build_dataset(dataset_list, transforms, dataset_catalog, is_train=True, external_proposal=False, old_classes=None, new_classes=None, excluded_classes=None,
    ...
    for dataset_name in dataset_list:
        ...
        if data["factory"] == "PascalVOCDataset2012":
            ...
            args["excluded_classes"] = excluded_classes
            args["is_train"] = is_train
            args["remove_old_classes"] = remove_old_classes
...
def make_data_loader(cfg,
                     ...
                     rank=0,
                     final_ids = None,
                     override_images_per_batch = 0,
                     remove_old_classes=True): #override cfg
    if is_train:
        images_per_batch = cfg.SOLVER.IMS_PER_BATCH if override_images_per_batch == 0 else override_images_per_batch
        ...
        if compression_not_shuffle:
            ...
            shuffle = True
        num_iters = cfg.SOLVER.MAX_ITER if override_images_per_batch == 0 else None
    else:
        images_per_batch = cfg.TEST.IMS_PER_BATCH if override_images_per_batch == 0 else override_images_per_batch
    ...
    for i, dataset in enumerate(datasets):
        if final_ids is not None and is_train:
           dataset.final_ids = final_ids#
