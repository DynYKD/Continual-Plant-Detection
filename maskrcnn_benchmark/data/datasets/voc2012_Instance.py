...
class PascalVOCDataset2012(torchvision.datasets.coco.CocoDetection):
    ...
    def __init__(self, data_dir, ann_file, split, use_difficult, transforms=None, external_proposal=False, old_classes=[],
        ...
        self.use_al = self.al_params["use_al"]
        self.remove_old_classes = remove_old_classes

        self.final_ids, count = self.filter_ids(self.ids)
    ...
    def filter_ids(self, check_ids):
        ...
        for img_id in check_ids:
            ...
            if has_valid_annotation(anno):
                if self.is_train:
                    if check_if_insert(anno, self.new_classes):  # filtering images for new categories
                        count = count + 1
                        ids.append(img_id)
                else:
                    if check_if_insert(anno, self.new_classes+self.old_classes):  # filtering images for new categories
                        count = count + 1
                        ids.append(img_id)
        return ids, count
    ...
    def __getitem__(self, index, shouldTransform=True):
        ...
        if self.is_train and self.remove_old_classes:
