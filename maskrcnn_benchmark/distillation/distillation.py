...
def calculate_mask_distillation_losses(soften_mask_logits, target_mask_logits):
    # Shouldn't we remove the background ? 
    #import pdb
    #pdb.set_trace()
    ...
    #print("labels", labels[0, :, 7, 7])
    #print(torch.sigmoid(old_classes_target_mask_logits)[0,:, 7, 7])
...
def unce_mask_distillation_losses(soften_mask_logits, target_mask_logits):  
    num_of_distillation_categories = soften_mask_logits.size()[1]
    tot_classes = target_mask_logits.shape[1]
    new_bkg_idx = torch.tensor([0] + [x for x in range(num_of_distillation_categories, tot_classes)]).to(target_mask_logits.device)
    outputs_no_bkg = target_mask_logits[:, 1:-(tot_classes-num_of_distillation_categories)]
    outputs_bkg = torch.sum(torch.index_select(target_mask_logits, index=new_bkg_idx, dim=1), dim=1).unsqueeze(1)

    modified_target_mask_logits = torch.cat([outputs_bkg, outputs_no_bkg], dim=1)

    labels = torch.sigmoid(soften_mask_logits)
    
    #print("unce", torch.sigmoid(modified_target_mask_logits)[0,:, 7,7])
    mask_distillation_loss = nn.functional.binary_cross_entropy_with_logits(modified_target_mask_logits, labels)
    return mask_distillation_loss


def unce_mask_distillation_losses_softmax(soften_mask_logits, target_mask_logits):  
    num_of_distillation_categories = soften_mask_logits.size()[1]
    tot_classes = target_mask_logits.shape[1]
    new_bkg_idx = torch.tensor([0] + [x for x in range(num_of_distillation_categories, tot_classes)]).to(target_mask_logits.device)
    den = torch.logsumexp(target_mask_logits, dim=1)
    outputs_no_bgk = target_mask_logits[:, 1:-(tot_classes-num_of_distillation_categories)] - den.unsqueeze(dim=1)
    outputs_bkg = torch.logsumexp(torch.index_select(target_mask_logits, index=new_bkg_idx, dim=1), dim=1) - den
    labels = torch.softmax(soften_mask_logits, dim=1)
    # make the average on the classes 1/n_cl \sum{c=1..n_cl} L_c
    loss = (labels[:, 0] * outputs_bkg + (labels[:, 1:] * outputs_no_bgk).sum(dim=1)) / soften_mask_logits.shape[1]
    mask_distillation_loss = -torch.mean(loss)
    return mask_distillation_loss
