...
def adapt_features_for_rehearsal(features, feature_rehearsal):  
    
    old_features, old_masks = feature_rehearsal
    
    resized_features = []
    for i in range(len(old_features)):
        resized_features.append(torch.nn.functional.interpolate(old_features[i], features[0].shape[2:]).squeeze(0))

    resized_features = torch.stack(resized_features, dim=0)
    small_old_masks = []
    
    
    for i in range(features[0].shape[0]):
        small_old_masks.append(torch.nn.functional.interpolate(old_masks[i].unsqueeze(0).unsqueeze(0), features[0].shape[2:]))
    small_old_masks = torch.stack(small_old_masks, dim=0).squeeze(1)
    #features = [features[0] * torch.logical_not(small_old_masks).int() + small_old_masks.repeat(1, 1024, 1, 1) * old_features.reshape(features[0].shape[0], -1, 1, 1)]
    features = [features[0] * torch.logical_not(small_old_masks).int() + small_old_masks.repeat(1, 1024, 1, 1) * resized_features]#.reshape(features[0].shape[0], -1, 1, 1)]
    return features
...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, images, targets=None, rpn_output_source=None, features=None, proposals=None, feature_rehearsal=None):
        """
        Arguments:
        ...
        if features is not None and proposals is not None:
            if feature_rehearsal is not None:
                features = adapt_features_for_rehearsal(features, feature_rehearsal)
    ...
    def generate_soften_proposal(self, images, targets=None, feature_rehearsal=None, features=None):
        backbone_features=[]
