...
cls=1
name="dynykd_b4_lr0001_rpn_cls${cls}_2.5K"
for s in {1..4};
do
    python -m torch.distributed.launch --master_port=${port} --nproc_per_node=${GPU} tools/train_incremental_same_backbone.py -t ${task} -n ${name} --rpn -s $s --dataset strawberry_diseases
done