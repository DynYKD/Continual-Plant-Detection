    ...
    for iteration, (images, targets, _, idx) in tqdm(enumerate(data_loader, start_iter), total=len(data_loader)):
        ...
        with torch.no_grad():
            soften_result, soften_mask_logits, soften_proposal, feature_source, _, _, rpn_output_source, roi_align_features_source = \
                model_source.generate_soften_proposal(images)  # feature_source, rpn_output_source and roi_align_features_source are not really used
            
        loss_dict_target, feature_target, _, _, rpn_output_target, target_proposals, _, target_soften_results \
            = model_target(images, targets, rpn_output_source=rpn_output_source) # rpn_output_source is useless (not actually used in function)
...
def main():
    ...
    if args.step > 0 and cfg_source.CLS_PER_STEP != -1:
        ...
        print("excluded classes:", cfg_target.MODEL.ROI_BOX_HEAD.NAME_EXCLUDED_CLASSES)
        ...
        print("New classes:", cfg_target.MODEL.ROI_BOX_HEAD.NAME_NEW_CLASSES)
