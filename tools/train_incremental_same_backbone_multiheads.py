...
def do_train(model_base, model_source, model_target, data_loader, optimizer, scheduler, checkpointer_source, checkpointer_target,
    ...
    model_base.eval()  # set the source model in inference mode
    ...
    for iteration, (images, targets, _, idx) in tqdm(enumerate(data_loader, start_iter), total=len(data_loader)):
        ...
        loss_dict_target, feature_target, _, _, rpn_output_target, target_proposals, _, target_soften_results = model_target(images, targets) # rpn_output_source is useless (not actually used in function)
        ...
        with torch.no_grad():
            soften_result, soften_mask_logits, soften_proposal, feature_source, _, _, rpn_output_source, roi_align_features_source = \
                model_source.generate_soften_proposal(images, features=feature_target)  # feature_source, rpn_output_source and roi_align_features_source are not really used
        #import pdb
        #pdb.set_trace()
        with torch.no_grad(): base_soften_result, base_soften_mask_logits, base_soften_proposal, feature_base, _, _, rpn_output_base, roi_align_features_base = model_base.generate_soften_proposal(images, features=feature_target) 

        #proposals = [BoxList(torch.cat([base_soften_proposal[i].bbox, soften_proposal[i].bbox]), soften_proposal[i].size) for i in range(len(soften_proposal))]
        ...
        target_result_base, target_mask_logits_base, _ = model_target.forward(images, targets,
                                                                                            features=feature_target,
                                                                                            proposals=base_soften_proposal)          
        
        if cfg.DIST.CLS > 0:
            distillation_losses = cfg.DIST.CLS * calculate_roi_distillation_losses(soften_result, target_result, dist=dist_type)
            distillation_losses += cfg.DIST.CLS * calculate_roi_distillation_losses(base_soften_result, target_result_base, dist=dist_type)
        else:
            distillation_losses = torch.tensor(0.).to(device)

        if cfg.MODEL.MASK_ON and cfg.DIST.MASK > 0:
            distillation_losses += 0.5 * cfg.DIST.MASK * calculate_mask_distillation_losses(soften_mask_logits, target_mask_logits)
            distillation_losses += 0.5 * cfg.DIST.MASK * calculate_mask_distillation_losses(base_soften_mask_logits, target_mask_logits_base)

        if cfg.DIST.RPN:
            rpn_distillation_losses = 0.5 * calculate_rpn_distillation_loss(rpn_output_source, rpn_output_target, cls_loss='filtered_l2', bbox_loss='l2', bbox_threshold=0.1)
            base_rpn_distillation_losses = 0.5 * calculate_rpn_distillation_loss(rpn_output_base, rpn_output_target, cls_loss='filtered_l2', bbox_loss='l2',bbox_threshold=0.1)
            distillation_losses += rpn_distillation_losses + base_rpn_distillation_losses
...
def train(cfg_source, cfg_target, cfg_base, logger_target, distributed, num_gpus, local_rank, remove_old_classes=True):
    print(cfg_source)
    model_base = build_detection_model(cfg_base)
    ...
    model_base.to(device)  # move source model to gpu
    ...
    checkpointer_base = DetectronCheckpointer(cfg_base, model_base, optimizer=None, scheduler=None,
    ...
    extra_checkpoint_data_base = checkpointer_base.load(cfg_base.MODEL.WEIGHT)
    ...
    do_train(model_base, model_source, model_target, data_loader, optimizer, scheduler, checkpointer_source, checkpointer_target,
...
def main():
    ...
    if args.step > 0 and cfg_source.CLS_PER_STEP != -1:
        ...
        cfg_base.MODEL.ROI_BOX_HEAD.NUM_CLASSES = len(cfg_source.MODEL.ROI_BOX_HEAD.NAME_OLD_CLASSES) + 1
    else:
        cfg_source.MODEL.ROI_BOX_HEAD.NUM_CLASSES = len(cfg_source.MODEL.ROI_BOX_HEAD.NAME_OLD_CLASSES) + 1
    ...
    train(cfg_source, cfg_target, cfg_base, logger_target, distributed, num_gpus, args.local_rank,
